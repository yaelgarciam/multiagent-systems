...
# cambios: 30 cajas y sin apilar (max_stack_initial = 1)
...
# Give robots a chance to compute initial paths so they start moving immediately
for rb in robots:
    rb.update(robots, {})  # empty allowed_map initially

TIME_LIMIT_MS = 90_000  # ahora 90 segundos
...
while running:
    ...
    # compute intentions: what each robot intends to move into this frame
    intentions = {}  # cell -> list of robots wanting it
    for rb in robots:
        if rb.path:
            target = tuple(rb.path[0])
            intentions.setdefault(target, []).append(rb)

    # decide which robot is allowed for contested cells
    # allowed_map: cell -> robot_id allowed to go there (tie-breaker by min id)
    allowed_map = {}
    for cell, lst in intentions.items():
        if len(lst) == 1:
            allowed_map[cell] = lst[0].id
        else:
            # tie-breaker: choose robot with smallest id (deterministic)
            chosen = min(lst, key=lambda r: r.id)
            allowed_map[cell] = chosen.id

    # update robots (pass global robots list and allowed_map)
    for rb in robots:
        prev_pos = (rb.r, rb.c)
    ...
    # check completion
    remaining_outside = any(warehouse[r][c] > 0 and (r,c) not in destination_cells
    ...
    # if no boxes left outside and piles full, force formation
    ...
    # if finished successfully (formed and all_full) -> popup and end
